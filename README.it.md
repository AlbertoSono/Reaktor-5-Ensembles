# 🎹 Collezione di Ensemble Reaktor 5
[Deutsch](README.md) | [English](README.en.md) | [Français](README.fr.md) | [Español](README.es.md) | [Português](README.pt.md) | [Italiano](README.it.md) | [日本語](README.ja.md)

Benvenuti in questo archivio di **Ensemble Native Instruments Reaktor 5**. Un progetto dedicato alla conservazione della creatività della comunità Reaktor.

## 🚀 Contenuto
- Oltre 400 file (circa 6 GB) tra sintetizzatori, effetti e sequencer.

## 📦 Utilizzo
- **Requisiti:** NI Reaktor 5 o superiore.
- **Nota:** Usiamo **Git LFS**. Per scaricare tutto correttamente, usa [GitHub Desktop](https://desktop.github.com).

## ⚖️ Note Legali
Archivio di **conservazione non commerciale**. Tutti i diritti appartengono ai rispettivi autori originali.
